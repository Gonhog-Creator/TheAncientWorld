...
public class SpeedRing extends Item {
    ...
    public SpeedRing(Properties properties) {
    ...
    public void inventoryTick(ItemStack itemStack, World world, Entity entity, int i, boolean flag) {
        ...
        if (!world.isRemote) {
            if (potionHandler.returnPotionLevel(playerEntity, RegistryHandler.SPEED_RING.get()) == 2) {
                if (potionHandler.isPotionActive(playerEntity, Effects.SPEED, 1)) {
                    return;
                } else {
            ...
            if (potionHandler.returnPotionLevel(playerEntity, RegistryHandler.SPEED_RING.get()) == 1) {
